...
cameraTrigger.onclick = function () {
  ...
  if (buttonSend) buttonSend.style.display = "";
...
function sendHttp(srcImage, url) {
  ...
  Http.send(
    JSON.stringify({ image: reduced.replace("data:image/jpeg;base64,", "") }),
  );
...
function displayProducts(products) {
  ...
  if (mode == "ingestion") {
    label1 = "Wybierz produkt z listy";
...
function submitProduct() {
  ...
  displayMessage("Dziękuję!<br>[" + product + "]");

  // console.log('submitProduct in action');
  productPicker.style.display = "none";
  // buttonSend.style.display="none";

  if (buttonSend) buttonSend.style.display = "none";
...
var brokerHost = window.location.hostname.replace(
  "camel-edge",
  "broker-amq-mqtt",
);
...
clientMqtt = new Paho.MQTT.Client(
  brokerHost,
  Number(brokerPort),
  "CameraClient-" + uid,
);
